...
async def handle_index(request):
    ...
    <html>
    <head>
        <title>Nostr-ntfy Settings</title>
        <style>
    ...
    <body>
        <div class="card">
            <h2>Nostr Bridge Settings</h2>
            <form action="/save" method="post">
                <label>Nostr Public Key (npub or hex):</label>
                <input type="text" name="pubkey" value="{config.get('pubkey', '')}" placeholder="npub1..." required>
                
                <label>Local ntfy URL:</label>
                <input type="text" name="ntfy_url" value="{config.get('ntfy_url', 'http://umbrel.nvpn:13199/nostr-events')}" required>
                
                <button type="submit">Save & Restart Listener</button>
            </form>
        </div>
    </body>
    </html>
...
async def handle_save(request):
    ...
    return web.Response(text="<script>alert('Settings Saved! Restarting bridge...'); window.location='/';</script>", content_type='text/html')
...
async def fetch_metadata(pubkey):
    ...
    print(f"Bootstrapping network topology for {pubkey[:8]}...")
    
    for relay in BOOTSTRAP_RELAYS:
        try:
            async with websockets.connect(relay) as ws:
                req = ["REQ", f"boot-{int(time.time())}", {"authors": [pubkey], "kinds": [3, 10002, 10009, 9021]}]
    ...
    while True:
        try:
            async with websockets.connect(relay_url) as ws:
                print(f"Listening on {relay_url}")
                current_time = int(time.time())
                sub_req = ["REQ", f"listen-{pubkey[:6]}", {"#p": [pubkey], "since": current_time}]
